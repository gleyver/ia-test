...
import type { PDF2PicResult } from "../../shared/types/types.js";
...
export class TesseractOCRService implements IOCRService {
  private static globalOCRWorker: Worker | null = null;
  private static workerInitializationPromise: Promise<Worker> | null = null;
  ...
  async extractText(pdfBuffer: Buffer): Promise<string> {
    ...
    try {
      ...
      if (allTexts.length > 0) {
        logger.info(
          { textLength: allTexts.join("\n\n").length, numPages },
          "OCR extraiu texto com sucesso"
        );
        return allTexts.join("\n\n").trim();
      }
   ...
   */
  private async processWithPdf2Pic(
    tempPdfPath: string,
    numPages: number,
    worker: Worker
  ): Promise<string[]> {
    ...
    logger.debug(
      { concurrentPages: CONCURRENT_PAGES, numCpus },
      "Configuração de concorrência OCR (pdf2pic)"
    );
    ...
    for (let i = 1; i <= numPages; i += CONCURRENT_PAGES) {
      ...
      const results = await Promise.all(
        batch.map((pageNum) => this.processPageWithPdf2Pic(pageNum, convert, worker, numPages))
      );
      allTexts.push(...results.filter((text): text is string => text !== null));
    }
   ...
   */
  private async processPageWithPdf2Pic(
    pageNum: number,
    convert: ReturnType<typeof fromPath>,
    worker: Worker,
    numPages: number
  ): Promise<string | null> {
    try {
   ...
   */
  private async processWithPdfJs(
    pdfBuffer: Buffer,
    numPages: number,
    worker: Worker
  ): Promise<string[]> {
    ...
    logger.debug(
      { concurrentPages: CONCURRENT_PAGES, numCpus },
      "Configuração de concorrência OCR (pdfjs)"
    );
    ...
    for (let i = 1; i <= numPages; i += CONCURRENT_PAGES) {
      const batch = this.createPageBatch(i, CONCURRENT_PAGES, numPages);
      const results = await Promise.all(
        batch.map((pageNum) => this.processPageWithPdfJs(pageNum, pdfDoc, worker, numPages))
      );
      allTexts.push(...results.filter((text): text is string => text !== null));
    }
   ...
   */
  private async processPageWithPdfJs(
    pageNum: number,
    ...
    try {
      ...
      try {
        ...
        if (textItems && textItems.trim().length > 0) {
          logger.debug(
            { pageNum, textLength: textItems.length, method: "text-only" },
            "Página processada (apenas texto)"
          );
          return `=== Página ${pageNum} ===\n${textItems.trim()}`;
        }
        return null;
   ...
   */
  private createPageBatch(start: number, size: number, max: number): number[] {
    const batch: number[] = [];
