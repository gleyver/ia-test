...
import type { TransformersOutput, TransformersPipeline } from "../shared/types/types.js";
...
export class EmbeddingGenerator implements IEmbeddingGenerator {
  ...
  private static sharedPipeline: TransformersPipeline | null = null;
  private static sharedModel: string | null = null;
  ...
  private distributedCache = getDistributedEmbeddingCache();

  private constructor({ model = "Xenova/all-MiniLM-L6-v2" }: { model?: string } = {}) {
    this.model = model;
    this.cache = new Map();
  ...
  }: { model?: string } = {}): EmbeddingGenerator {
  ...
  async initialize(): Promise<TransformersPipeline> {
    ...
    if (!EmbeddingGenerator.sharedPipeline || EmbeddingGenerator.sharedModel !== this.model) {
      ...
      EmbeddingGenerator.sharedPipeline = (await pipeline(
        "feature-extraction",
        this.model
    ...
    if (textsToProcess.length > 0) {
      for (let i = 0; i < textsToProcess.length; i += BATCH_SIZE) {
        ...
        const outputs = (await pipe(textsToProcessArray, {
          pooling: "mean",
          normalize: true,
        })) as TransformersOutput | TransformersOutput[];
        ...
        for (let idx = 0; idx < batch.length; idx++) {
          ...
          const embedding = Array.from(outputsArray[idx].data) as number[];
