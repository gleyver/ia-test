...
describe("ExecuteQueryUseCase", () => {
  ...
  let mockRetrieverFactory: (vectorDb: VectorDB) => IRetriever;
  let mockResponseGenerator: IResponseGenerator;
  ...
  beforeEach(() => {
    ...
    mockRetrieverFactory = vi.fn().mockReturnValue(mockRetriever as IRetriever);
    ...
    } as unknown as IResponseGenerator;
  ...
  describe("execute - com arquivo e VectorDB", () => {
    it("deve buscar documentos relevantes e gerar resposta com contexto", async () => {
      ...
      expect(mockResponseGenerator.generate).toHaveBeenCalledWith(
        "Qual é o conteúdo?",
    ...
    it("deve usar conhecimento do modelo quando não encontrar documentos", async () => {
      ...
      expect(mockResponseGenerator.generateWithoutContext).toHaveBeenCalledWith(
        "Qual é o conteúdo?"
      );
