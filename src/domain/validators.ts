...
const ALLOWED_EXTENSIONS = [".pdf", ".docx", ".txt", ".html", ".htm"] as const;
...
export function validateExtension(filename: string): void {
  ...
  if (!ALLOWED_EXTENSIONS.includes(ext as (typeof ALLOWED_EXTENSIONS)[number])) {
    throw new ValidationError(
      `Extensão não suportada: ${ext}. Extensões permitidas: ${ALLOWED_EXTENSIONS.join(", ")}`
    );
  }
}
 ...
 */
export async function validateMimeType(buffer: Buffer, filename: string): Promise<void> {
  ...
  if (!fileType) {
    ...
    throw new ValidationError(
      "Não foi possível determinar o tipo do arquivo. Arquivo pode estar corrompido."
    );
  }

  // Validar tipo MIME real
  if (!ALLOWED_MIME_TYPES.includes(fileType.mime as (typeof ALLOWED_MIME_TYPES)[number])) {
    throw new ValidationError(
      `Tipo de arquivo não suportado: ${fileType.mime}. Tipos permitidos: ${ALLOWED_MIME_TYPES.join(", ")}`
    );
  }
}
 ...
 */
export async function validateFile(buffer: Buffer, filename: string, size: number): Promise<void> {
  // 1. Validar tamanho
