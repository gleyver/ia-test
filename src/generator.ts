...
import { CircuitBreaker } from "./infrastructure/circuitBreaker/circuitBreaker.js";
 ...
 */
class OllamaPool {
  private queue: RequestQueue<GenerateResult>;
  ...
  constructor({
    ...
    // Usar composição
    this.queue = new RequestQueue<GenerateResult>(finalMaxConcurrent);
   ...
   */
  async request(prompt: string, retrievedDocs?: SearchResult[]): Promise<GenerateResult> {
  ...
  private async executeWithRetry(
    prompt: string,
    retrievedDocs: SearchResult[] | undefined,
    retryCount: number
        ...
        execute: async () => {
          try {
            ...
            const result = await this.circuitBreaker.execute(
              () => this.callOllama(prompt, retrievedDocs),
              () => {
            ...
            if (this.retryStrategy.shouldRetry(retryCount)) {
              ...
              logger.warn(
                { retryCount: retryCount + 1, maxRetries: this.retryStrategy["maxRetries"] },
                "Retry Ollama"
              );
  ...
  private async callOllama(
    ...
    try {
      ...
      const result = (await response.body.json()) as { response?: string };
...
export class ResponseGenerator implements IResponseGenerator {
  ...
  private static sharedPool: OllamaPool | null = null;
  ...
  static getInstance({
    model = "llama3.2",
    ollamaUrl = "http://localhost:11434",
  }: { model?: string; ollamaUrl?: string } = {}): ResponseGenerator {
  ...
  buildContext(retrievedDocs: SearchResult[]): string {
    ...
    return retrievedDocs
      .map((doc, i) => {
        ...
        const text =
          doc.text.length > maxDocLength ? doc.text.substring(0, maxDocLength) + "..." : doc.text;
...
RESPOSTA:`;
   ...
   */
  async generateWithoutContext(query: string): Promise<GenerateResult> {
