...
class DistributedRateLimiter {
    ...
    constructor({ windowMs = config.rateLimit.windowMs, maxRequests = config.rateLimit.maxRequests, } = {}) {
        ...
        if (!this.redis) {
            ...
            logger.info({ windowMs: this.windowMs, maxRequests: this.maxRequests }, "Rate limiter distribuído configurado (Redis)");
        }
    }
    /**
    ...
    async checkWithRedis(key) {
        ...
        try {
            ...
            logger.error({ error: errorMessage }, "Erro no rate limiter Redis, usando fallback em memória");
            this.fallbackToMemory = true;
            return this.checkWithMemory(key);
        }
    }
...
export function distributedRateLimitMiddleware() {
    ...
    return async (c, next) => {
        ...
        if (!result.allowed) {
            ...
            throw new RateLimitError(`Muitas requisições. Tente novamente após ${new Date(result.resetTime).toISOString()}`, {
