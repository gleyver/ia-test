...
export class PDFProcessorWithOCR extends PDFProcessor {
    ...
    async process(filePath) {
        ...
        const shouldUseOCR = result.text.length < 1000 || (result.text.length < 200 && (info.total || 0) > 0);
        if (shouldUseOCR) {
            logger.warn({ textLength: result.text.length, pages: info.total }, "PDF extraiu pouco texto. Tentando OCR...");
            try {
                ...
                if (ocrText && ocrText.length > 0) {
                    if (ocrText.length > result.text.length * 1.2) {
                        ...
                        logger.info({ ocrLength: ocrText.length, originalLength: result.text.length }, "OCR extraiu mais texto - usando OCR como principal");
                        result.text = ocrText;
                        result.metadata.usedOCR = true;
                    }
                    else {
                        // Combinar ambos
                        logger.info({ ocrLength: ocrText.length, originalLength: result.text.length }, "Combinando texto original com OCR");
...
//# sourceMappingURL=pdfProcessorWithOCR.js.map