    ...
    async extractTextWithOCR(pdfBuffer) {
        ...
        try {
            ...
            try {
                ...
                const errorMessage = pdf2picError instanceof Error ? pdf2picError.message : String(pdf2picError);
                console.log(`⚠️ pdf2pic não disponível (${errorMessage}), usando pdfjs-dist + canvas como fallback`);
                usePdf2Pic = false;
            }
            const allTexts = [];
            if (usePdf2Pic) {
                ...
                for (let pageNum = 1; pageNum <= numPages; pageNum++) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            console.warn(`  ⚠️ Erro na renderização completa (${errorMessage}), tentando extrair apenas texto...`);
                    ...
                    if (shouldUseOCR) {
                        ...
                        if (ocrText && ocrText.length > 0) {
                            ...
                            if (ocrText.length > text.length * 1.2) {
                                ...
                                console.log(`✅ OCR extraiu ${ocrText.length} caracteres (vs ${text.length} do método normal)`);
                                text = ocrText;
