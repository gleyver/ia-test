...
import { createMockEmbeddingGenerator, createMockSearchResult, createMockVectorDB, } from "../../helpers/mocks.js";
describe("Retriever", () => {
    let retriever;
    let mockVectorDB;
    ...
    describe("retrieve", () => {
        it("deve buscar documentos relevantes para query", async () => {
            ...
            vi.spyOn(mockEmbeddingGenerator, "generateEmbedding").mockResolvedValue(new Array(384).fill(0.1));
        ...
        it("deve usar topK padrão quando não especificado", async () => {
            ...
            expect(mockVectorDB.search).toHaveBeenCalledWith(expect.any(Array), expect.objectContaining({ topK: 10 }));
        });
        it("deve usar topK customizado", async () => {
            vi.spyOn(mockVectorDB, "search").mockResolvedValue([]);
            vi.spyOn(mockEmbeddingGenerator, "generateEmbedding").mockResolvedValue(new Array(384).fill(0.1));
            await retriever.retrieve("teste", { topK: 5 });
            expect(mockVectorDB.search).toHaveBeenCalledWith(expect.any(Array), expect.objectContaining({ topK: 5 }));
        });
        it("deve passar filter para VectorDB", async () => {
            ...
            expect(mockVectorDB.search).toHaveBeenCalledWith(expect.any(Array), expect.objectContaining({ filter }));
        });
