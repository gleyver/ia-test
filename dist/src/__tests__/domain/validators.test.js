...
import { sanitizeFilename, sanitizeQuery, validateExtension, validateFile, validateFileSize, validateMimeType, } from "../../domain/validators.js";
...
describe("validators", () => {
    ...
    describe("validateMimeType", () => {
        ...
        it("deve rejeitar tipo MIME não suportado", async () => {
            ...
            await expect(validateMimeType(Buffer.from("test"), "arquivo.zip")).rejects.toThrow(ValidationError);
        });
        ...
        it("deve rejeitar arquivo sem tipo detectável (exceto texto)", async () => {
            const { fileTypeFromBuffer } = await import("file-type");
            vi.mocked(fileTypeFromBuffer).mockResolvedValue(undefined);
            await expect(validateMimeType(Buffer.from("test"), "arquivo.pdf")).rejects.toThrow(ValidationError);
        });
    });
    describe("sanitizeQuery", () => {
        ...
        it("deve rejeitar query não string", () => {
            expect(() => sanitizeQuery(null)).toThrow(ValidationError);
            expect(() => sanitizeQuery(123)).toThrow(ValidationError);
        });
    ...
    describe("validateFile", () => {
        ...
        it("deve rejeitar arquivo muito grande", async () => {
            const buffer = Buffer.from("test");
            await expect(validateFile(buffer, "arquivo.pdf", FileSize.MAX_BYTES + 1)).rejects.toThrow(ValidationError);
        });
