...
const ENV_RULES = [
    ...
    },
    {
        ...
        errorMessage: "JWT_SECRET deve ter pelo menos 32 caracteres e não pode ser 'change-me-in-production' em produção",
    },
    {
        key: "PORT",
        required: false,
...
export function validateRequiredEnvVars() {
    ...
    for (const rule of ENV_RULES) {
        ...
        if (!value && rule.defaultValue) {
            ...
            logger.info({ key: rule.key, defaultValue: rule.defaultValue }, "Usando valor padrão para variável de ambiente");
            continue;
        }
...
function validateAdditionalRules() {
    ...
    if (nodeEnv === "production") {
        ...
        if (jwtSecret === "change-me-in-production" || jwtSecret.length < 32) {
            throw new Error("JWT_SECRET em produção deve ter pelo menos 32 caracteres e não pode ser o valor padrão");
        }
    }
