 ...
 */
export function cosineSimilarity(vecA, vecB, normA, normB) {
    ...
    for (let i = 0; i < vecA.length; i++) {
        ...
        if (!normA)
            computedNormA += vecA[i] * vecA[i];
        if (!normB)
            computedNormB += vecB[i] * vecB[i];
    }
    // Early exit: se dotProduct muito negativo, similarity será baixa
    // Isso ajuda a pular cálculos desnecessários
    const maxPossibleSimilarity = Math.abs(dotProduct) / (Math.sqrt(computedNormA) * Math.sqrt(computedNormB));
 ...
 */
export function calculateNorm(vec) {
    let sum = 0;
